...
public class RandomizationSettings
{
    ...
    [ControlsField]
    [MenuRange(2, MAX_REVEALS)]
    public int NumberOfReveals = 4;

    [ControlsField]
    public TieBreakerOrder TieBreaks = TieBreakerOrder.GoodItemsFirst;

    public bool AltarOfDivination = false;

    [AltarField]
    public bool CurseOfWeakness = true;

    [AltarField]
    public bool CurseOfObsession = true;

    [AltarField]
    public bool CurseOfTheDamned = false;
    ...
    {
        if (
            item.name == ItemNames.Simple_Key
            || item.name == ItemNames.Collectors_Map
            || item.name == ItemNames.Godtuner
        )
            return false;
    ...
    private static readonly MetadataProperty<AbstractItem, string> TreasureHuntGroup = new(
        "TreasureHuntGroup",
        _ => None
    );
    ...
    public bool IsTrackedItem(AbstractItem item)
    {
        ...
        if (injectedGroup != None)
            return IsGroupEnabled(injectedGroup);

        if (baseGroups.TryGetValue(item.name, out var baseGroup))
            return IsGroupEnabled(baseGroup);
        else
